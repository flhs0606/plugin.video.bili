...
2. **`xbmc.service` → [service.py](service.py)**: long-running service. Builds a [BilibiliMonitor](monitor.py) (Kodi `xbmc.Monitor` subclass) and loops on `waitForAbort`. Hosts the local HTTP server (serves generated `.mpd` files and proxies CDN segment requests).
...
- [playback/](playback/) — VOD playback helpers:
  - [playback/proxy.py](playback/proxy.py) — 本地 CDN 代理注册表：`register(base_url, cookie, ua)` → seg_id，`lookup(seg_id)` → (url, headers)。MPD BaseURL 写 `http://127.0.0.1:PORT/proxy/{id}.mp4`，http_server 收到请求后查 registry 转发到 B 站 CDN。
  - [playback/mpd.py](playback/mpd.py) — `generate_mpd(dash, cookie, ua, port)`: 生成标准 MPEG-DASH MPD。每个 Representation 的 `<BaseURL>` 指向本地代理（注入认证头）。使用纯 BaseURL 模式（无 SegmentBase/SegmentTemplate），ffmpeg dash demuxer 原生支持。多 AdaptationSet 保留：视频 AS（SDR/HLG/HDR10/DV 分组）+ 音频 AS（AAC/Atmos/Hi-Res FLAC）。
  ...
  - [playback/m3u8.py](playback/m3u8.py) — (保留备用) HLS playlist 生成器。当前默认使用 MPD 路径，此模块不再被调用。
...
- [monitor.py](monitor.py) — `BilibiliMonitor(xbmc.Monitor)`: owns the local HTTP server, cleans `special://temp/plugin.video.bili/` on startup. `restart_httpd()` exists but is **not** called by `service.py` (the port is fixed at addon settings load time).
- [http_server.py](http_server.py) — `http.server.HTTPServer` bound to `0.0.0.0:54321` (overridable via `server_port` setting). Two endpoints: (1) serve generated `.mpd` files from `special://temp/plugin.video.bili/`, (2) `/proxy/{id}.mp4` — CDN reverse proxy that injects Cookie/Referer/UA and stream-forwards B站 CDN responses. Path-traversal guard (`_safe_file_path`).
...
- **Proxy registry is in-memory only** — `playback/proxy.py` stores CDN URL→seg_id mappings in a module-level dict. Since Kodi spawns a fresh Python process per navigation, stale registries are auto-cleaned. The `unregister_all()` call at the start of `video()` is a safety net.
