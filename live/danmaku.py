        ...
        def _writer():
            ...
            while self.running:
                ...
                if not self.running:
                    break
                with self.lock:
                    if len(self.danmaku_list) > self.MAX_LIST:
                        self.danmaku_list = self.danmaku_list[-self.MAX_LIST:]
                    now_offset = time.time() - self._start_time
                    cutoff = now_offset - self.stay_time - 2
                    pool = self.danmaku_list[-self.MAX_LIST:]
                    snapshot = [c for c in pool if c[0] >= cutoff]
                    if len(snapshot) < 50:
                        snapshot = list(self.danmaku_list[-50:])
                    snapshot = snapshot[-self.MAX_LIST:]
